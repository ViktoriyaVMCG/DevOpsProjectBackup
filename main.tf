...
resource "aws_instance" "dev" {
  ...
  availability_zone = "us-east-1c"
  key_name = aws_key_pair.generated_key.key_name
  security_groups = [aws_security_group.customer_sg.name, aws_security_group.employee_sg.name, aws_security_group.jump_sg.name]
...
#  depends_on = [aws_instance.dev,aws_instance.prod,aws_instance.jenkins,local_file.inventory_ansiblessh,local_file.jump_ansiblessh]
#
#  provisioner "local-exec" {
#    command =  "ansible-playbook -i inventory jump.yml && ansible-playbook -i inventory devops.yml && ansible-playbook -i inventory jenkins.yml"
#  }
#}
